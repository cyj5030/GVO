...
class KittiEvalOdom():
    ...
    def eval(self, gt_txt, result_txt, seq=None, stride=1):
        ...
        # for k, v in poses_gt.items():
        #     if int(k)%4 == 0:

        poses_gt = {int(k)//stride:v for k, v in poses_gt.items() if int(k)%stride == 0}
...
def trace_handler(prof):
    ...
    prof.export_stacks('./perf_viz')

@torch.no_grad()
def eval_kitti_odometry(net, datasets, logger, cfgs, kpts=None, write=True):

    device = cfgs['device']
    net = net.eval()
    for dataset in datasets:
        if 'Odometry' not in dataset._name:
            continue
        ...
        for i, data in tqdm(enumerate(dataloader)):
            ...
            # visual
            if write:
                pathname = os.path.join(logger.root, 'raw_data')
                if not os.path.exists(pathname):
                    os.mkdir(pathname)
                np.savez(os.path.join(pathname, str(i).zfill(6) + '.npz'), 
                        # flow=eval_data['flow'],
                        disp=eval_data['disp'],
                        sample_points=eval_data['sample_points'],
                        ba_disp = eval_data['ba_disp'])

                # for key, value in eval_data.items():
                #     if key == 'disp_update':
                #         continue
                #     if key == 'sample_points':
                #         write_match(data, value, logger, folder='vo_' + key, name=str(i).zfill(6))
                #     else:
                #         write_data(value, logger, folder='vo_' + key, name=str(i).zfill(6))

        if dataset.eval_nums is None:
            eval_seq = dataset.sequences[0]
            save_name = 'kitti_' + eval_seq + dataset.save_prefix
            result_txt = logger.save_traj(poses, prefix=save_name)
            eval_tool = KittiEvalOdom()

            loss_pack, poses_gt, poses_result = eval_tool.eval(dataset.gt_pose_file, result_txt, seq=eval_seq, stride=dataset.stride)
            logger.print_eval_pose(loss_pack, prefix='odometry_' + save_name, write=True)
            logger.plotPath(save_name, poses_gt, poses_result)
        print('END!')

if __name__ == "__main__":
    gt_pose = '/home/cyj/datasets/KITTI/data_odometry_color/poses/09.txt'
    pred_pose = '/home/cyj/code/VO/debug/Test-vo-lowFPS_Log/09.txt'
    eval_tool = KittiEvalOdom()
    loss_pack, poses_gt, poses_result = eval_tool.eval(gt_pose, pred_pose, seq='09', stride=3)
    print(loss_pack)